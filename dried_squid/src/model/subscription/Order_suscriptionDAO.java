...
//	String sql_insert = "insert into order_subscription values ((select nvl(max(SUBSCRIPTION_NUMBER),0)+1 from ORDER_SUBSCRIPTION os),?,?,(select sysdate from dual),?,(select sysdate from dual),(select SYSDATE + (INTERVAL '30' DAY) from dual),?)";
	...
	public Product_setVO selectRandom(Order_subscriptionVO vo) {
		...
		try {
			if (vo.getSoup_check().equals("미포함")) {
				String sql = "select product_set_number from product_set where soup_check='미포함' ORDER BY rand() LIMIT 1";
				// soup_check="미포함" 조건인 product_set_number 중 랜덤으로 1개의 번호 추출
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
			} else {
				String sql = "select product_set_number from product_set where soup_check='포함' ORDER BY rand() LIMIT 1";
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
			}
	...
	public boolean insert(Order_subscriptionVO vo) {
		conn = JDBCUtil.connect();
